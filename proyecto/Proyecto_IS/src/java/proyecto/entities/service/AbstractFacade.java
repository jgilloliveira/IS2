...
// Usuario
    ...
    public String listarUserGrupo(int id) {
        ...
        + "on u.grupo = g.id_grupo where u.grupo = "+id+" and u.rol <> 4 order by u.id_usuario;");
        List resultado = q.getResultList();
        JSONArray jsonArray = new JSONArray();
        for (int i = 0; i < resultado.size(); i++) {
            JSONObject json = new JSONObject();
...
// Proyecto
    ...
    public String obtenerDatoProyecto(int id) {
        javax.persistence.Query q = getEntityManager().createNativeQuery("select nombre || ',' || descripcion "
        + "||','|| fecha_inicio ||','|| fecha_fin ||','|| estado from proyecto where id_proyecto = "+id+";");
        String resultado = (String) q.getSingleResult();
        JSONObject json = new JSONObject();
        String[] partes = resultado.split(",");
        json.put("nombre", partes[0]);
        json.put("descripcion", partes[1]);
        json.put("fechaInicio", partes[2]);
        json.put("fechaFin", partes[3]);
        json.put("estado", partes[4]);
        return json.toString();
    }
    
    public String editProyecto(Proyecto proyecto) {
        javax.persistence.Query q1 = getEntityManager().createNativeQuery("update proyecto set "
        + "nombre = '"+ proyecto.getNombre()+"', descripcion = '"+proyecto.getDescripcion()+"', "
        + "fecha_inicio = '"+proyecto.getFechaInicio()+"', fecha_fin = '"+proyecto.getFechaFin()+"', "
        + "estado = "+proyecto.getEstado().getIdEstado()+" where id_proyecto = "+proyecto.getIdProyecto()+";");
        q1.executeUpdate();
        javax.persistence.Query q2 = getEntityManager().createNativeQuery("select count(id_proyecto) "
        + "from proyecto where id_proyecto = "+proyecto.getIdProyecto()+" and nombre = '"+ proyecto.getNombre()+"' "
        + "and descripcion = '"+proyecto.getDescripcion()+"' and fecha_inicio = '"+proyecto.getFechaInicio()+"' "
        + "and fecha_fin = '"+proyecto.getFechaFin()+"' and estado = "+proyecto.getEstado().getIdEstado()+";");
        long resultado = (Long)q2.getSingleResult();
        JSONObject json = new JSONObject();
        if(resultado>0){
            json.put("respuesta", true);
...
// Grupo Proyecto
    ...
    public String obtenerDatoGrupo(int id) {
        javax.persistence.Query q = getEntityManager().createNativeQuery("select nombre || ',' || cantidad "
        + "from grupo_de_trabajo where id_grupo = "+id+";");
        String resultado = (String) q.getSingleResult();
        JSONObject json = new JSONObject();
        String[] partes = resultado.split(",");
        json.put("nombre", partes[0]);
        json.put("cantidad", partes[1]);
        return json.toString();
    }
    
    public String editGrupo(GrupoDeTrabajo grupo) {
        javax.persistence.Query q1 = getEntityManager().createNativeQuery("update grupo_de_trabajo set "
        + "nombre = '"+ grupo.getNombre()+"', cantidad = "+grupo.getCantidad()+" "
        + "where id_grupo = "+grupo.getIdGrupo()+";");
        q1.executeUpdate();
        javax.persistence.Query q2 = getEntityManager().createNativeQuery("select count(id_grupo) "
        + "from grupo_de_trabajo where id_grupo = "+grupo.getIdGrupo()+" and nombre = '"+ grupo.getNombre()+"' "
        + "and cantidad = "+grupo.getCantidad()+";");
        long resultado = (Long)q2.getSingleResult();
        JSONObject json = new JSONObject();
        if(resultado>0){
            json.put("respuesta", true);
...
// Sprint
    ...
    public String listarSprint(int id) {
        ...
        + "on p.id_proyecto = g.proyecto_grupo where g.id_grupo="+id+" order by s.id_sprint;");
        List resultado = q.getResultList();
        JSONArray jsonArray = new JSONArray();
        for (int i = 0; i < resultado.size(); i++) {
            JSONObject json = new JSONObject();
    ...
    public String obtenerProyecto(int id) {
        javax.persistence.Query q = getEntityManager().createNativeQuery("select proyecto_grupo "
        + "from grupo_de_trabajo where id_grupo = "+id+";");
        int resultado = (int)q.getSingleResult();
        JSONObject json = new JSONObject();
